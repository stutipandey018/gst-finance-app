...
const emptyProfile = {
  ...
  phone: '', email: '', bank_name: '', account_no: '',
  ifsc: '', upi: '', registration_type: 'Regular', business_type: 'Proprietorship',
}
  ...
  const loadProfile = async () => {
    ...
    if (data) {
      ...
      localStorage.setItem('gst_profile', JSON.stringify({
        businessName: data.business_name, ownerName: data.owner_name,
        gstin: data.gstin, pan: data.pan, address: data.address,
        city: data.city, state: data.state, pincode: data.pincode,
        phone: data.phone, email: data.email, bankName: data.bank_name,
        accountNo: data.account_no, ifsc: data.ifsc, upi: data.upi,
      }))
    }
  }
  ...
  const saveProfile = async () => {
    ...
    if (error) alert('Error: ' + error.message)
      ...
      localStorage.setItem('gst_profile', JSON.stringify({
        ...
        gstin: profile.gstin, address: profile.address, city: profile.city,
        state: profile.state, pincode: profile.pincode, phone: profile.phone,
        email: profile.email, bankName: profile.bank_name,
        accountNo: profile.account_no, ifsc: profile.ifsc, upi: profile.upi,
      }))
      setSaved(true)
      setErrors({})
    ...
    <div>
      ...
      {activeTab === 'password' && (
        <div className="max-w-md">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="font-semibold text-gray-700 mb-6 pb-2 border-b">Change Password</h3>
            <div className="space-y-4">
              <div>
